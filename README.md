...
如果带小数，也有非数字字符呢？这时候就得用<a href="#转换为数字">原生</a>的方法了。
```angularjs
...
## 数组排序（数字排序）
```angular2html
var arr = [22,10,43,53,73,3]
const arr2 = arr.sort((a, b) => a - b)
console.log(arr2) // [3, 10, 22, 43, 53, 73]
```
...
# 自造常用函数
## 将字符串分割为数字和非数字字符的数组
> eg: 将'3.246px'分割为 [3.246, 'px']
> 
```angular2html
    function segment(item){
      const num = parseFloat(item);
      const unit = item.replace(num, '');
      return [num, unit]
    }
    var ss = segment('7574.2646rem');
    console.log(ss); // [7574.2646, "rem"]
```
